...
% Algorithms and other environments
...
Such visualizations, however, are only informative if their aesthetics convey relationships. Consider, for instance, a disc with a radius of 2~cm labelled \emph{Men}\marginpar{%
<<men, fig.width=1.45, fig.height=1.45>>=
plot(euler(c(Men = 2)))
@
}---it says nothing by itself; yet if we juxtapose it with a 1~cm-radius disc labelled \emph{Children}\marginpar{%
<<children, fig.width=1, fig.height=1>>=
plot(euler(c(Children = 1)), fill = "white")
@
}, the graphic starts to become informative. Now, if we  then intersect the two discs, generating a visible overlap, we have successfully visualized the relative proportions of men, children, and their intersection. The diagram we have constructed is a \emph{Euler diagram}~(\cref{fig:children-men}).
...
\begin{itemize}
\item intersections and relative complements\sidenote{$A \setminus B = 3 \quad B \setminus A = 2 \quad A \cap B=1$},
\item unions and identities\sidenote{$A=4 \quad B=3 \quad A \cap B=1$},
\item a matrix of binary (or boolean) indices\sidenote{$\begin{bmatrix}\bm{A} & \bm{B} & \bm{C} \\0 & 1 & 0 \\1 & 1 & 1 \\1 & 0 & 0 \\ \end{bmatrix}$},
\item a list of sample spaces\sidenote{$\begin{matrix} A = \{ab,\,bb,\,bc\}\\B = \{aa,\,bc,\,cc\}\\C = \{bb,\,bb,\,cc\} \end{matrix}$}, or
\item a two- or three-way table \sidenote{\begin{tabular}{lrr} \toprule Survived? & No  & Yes \\   \midrule Child & 52.00 & 57.00 \\   Adult & 1438.00 & 654.00 \\    \bottomrule\end{tabular}}.
\end{itemize}
...
As we saw in the \nameref{sec:background}, we now need to have all the ellipses' points of intersections at hand. \pkg{eulerr}'s approach to this is outlined in \citet{richter-gebert_2011} and based in \emph{projective}, as opposed to \emph{euclidean}, geometry.
...
%
% \begin{algorithm}[htb]
% \sidecaption{The algorithm we use to simulate circles and ellipses, compute their areas, and use these to fit Euler diagrams using the different software packages.}
...
\begin{algorithm}[hbt]
\sidecaption{The algorithm used to simulate circles and ellipses, compute their areas, and fit Euler diagrams to these layouts using the different software packages.}
\For{$i\leftarrow 3$ \KwTo $8$}{
  \For{$j\leftarrow 1$ \KwTo $500$}{
...
%
\begin{figure}[bhtp]
\sidecaption{Reproducibility tests for ellipses and circles generated from the distributions from \eqref{eq:consistencyCircles}. Note that \pkg{Vennerable} only supports Euler diagrams for three sets, which is why its data is absent for the other cases.}
<<consistency, fig.height = 2.3, fig.width = 4.25, out.width = "\\linewidth", cache.extra = tools::md5sum(file.path("data", "data_consistency.rda"))>>=
data_consistency %>%
  spread(metric, loss) %>%
...
%
% \begin{algorithm}[htb]
% \sidecaption{The algorithm we use to simulate random set relationships and fit them with the software under study to assess their accuracy.}
% \label{alg:accuracy}
% \begin{algorithmic}[1]
%
% \FOR{$N=3,4,\dots,8$}
...
\begin{figure*}[hbt]
<<accuracy, fig.height = 3, fig.width = 6.6, out.width = "\\linewidth", cache.extra = tools::md5sum(file.path("data", "data_accuracy.rda"))>>=
data_accuracy %>%
  filter(!is.na(Loss)) %>%
...
\begin{figure*}[hbt]
<<performance, fig.height = 2.5, fig.width = 6.6, out.width = "\\linewidth", cache.extra = tools::md5sum(file.path("data", "data_performance.rda"))>>=
data_performance %>%
  filter(!is.na(Time)) %>%

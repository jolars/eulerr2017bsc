...
% Block margins for sidecaption around environments
...
\pkg{venn.js} uses residual sums of squares\sidenote{
...
}), \pkg{venneuler} uses the \emph{stress} metric\sidenote{\pkg{venneuler}'s
...
\pkg{Vennerable}
uses Chow's~\citep{Chow_2005} idealistic function\sidenote{
...
proportional loss function\sidenote{\pkg{eulerAPE}'s cost function is
...
pass the parameters on to a last-ditch optimizer. The weapon of choice\sidenote{
...
with ellipses and a diagError~\eqref{eq:diagError} above 0.001\sidenote{
...
\pkg{eulerAPE} have been placed at seemingly arbitrary locations\sidenote{In
...
fails for \Sexpr{filter(failures, software == "venn.js")$failures} sets out of
\Sexpr{filter(failures, software == "venn.js")$n} diagrams. This difference,
...
one\sidenote{The relevant line of code used to generate this tolerance is
...
\begin{figure}[hbtp]
\sidecaption{Tukey box plots of diagError and stress for Euler diagrams
...
optimizer in case the fit it not adequate\sidenote{Here, we define \emph{adequate}
...
diagrams, where it ranks best in terms of stress but not diagError\sidenote{
...
in contrast to us, people\sidenote{And the other

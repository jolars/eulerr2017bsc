...
knitr::opts_chunk$set(
  ...
  dev.args =
    if (capabilities("cairo") &&
        any(grepl("LinLibertine_R.otf", sysfonts::font.files(), fixed = TRUE)))
...
Visualizations are most effective when they show
...
of two sets. If we moreover proceed to intersect the two
discs, producing an overlap, we successfully visualize the
relative proportions of men and children, as well as their intersection. The diagram
we have constructed is an \emph{Euler diagram}\footnote{%
...
a generalization of the obiquiteous \emph{Venn diagram}, the latter being
a staple of introductory
text books in statistics and research disciplines such as biomedicine and
geology. Venn and Euler diagrams both displays
set relationships but differ in that the the former require all
...
surface of the diagram maps to some quantity. (This is the case with
the diagram to the right.) This is a rational form for a
Euler diagram---only its geometries are necessary to interpret it. It lets us, for
...
circles~\citep{Wilkinson_2012,Kestler_2008,Swinton_2011}. The latter is the
popular choice, and for good reason, since they are easiest to
interpret~\citep{Blake_2016}. Yet, circles do not always support
...
impossible with circles, given that we \emph{require} 15 intersections but with four circles
...
multi-dimensional scaling (MDS) with
Jacobian distances, taking only pairwise relationships into account.
\pkg{venn.js}~\citep{Frederickson_2016} combines a constrained version of the MDS
algorithm from \pkg{venneuler},
that is contrastingly based on euclidean distances, with a greedy
...
Diagrams with more than two sets normally require additional tuning, the
prerequisite for which is that
we first know the areas of the overlaps so that we can assess how well our
diagram fits the input. Computing overlaps, however, is no trivial task,
particularly not for ellipses. This is evident in that many methods resort to
approximations such as quadtrees~\citep{Wilkinson_2012} (\pkg{venneuler}) or polygon
intersections~\citep{Kestler_2008} (\pkg{VennMaster}).

Compared to the approximative algorithms, exact algorithms require that we
...
\citet{Micallef_2014a} (\pkg{eulerAPE}) has developed solutions for circles
and ellipses respectively---although the latter, as we previously noted,
is restricted to three intersecting ellipses. No algorithm has so far been
published that extends these methods to any number of ellipses or elliptical
three-set diagrams with subset of disjoint intersections.
...
\pkg{venn.js} uses the residual sums of squares\footnote{%
...
and $A_i$ the corresponding overlap's area in the diagram.}, \pkg{venneuler}
...
areas exist, making it inappropriate for algorithms that aims to fit
set configurations with either subset or disjoint relationships.

\pkg{venn.js} relies on a Nelder--Mead optimizer for the final optimization.
\pkg{venneuler}, on the other hand, runs a combination of
steepest descent optimizer with gradient approximation and
...
\begin{enumerate}
\item data,
\item a model---in this case a diagram---to fit the data with,
...
\]
Analogously to $\omega$, we also introduce the $\&$
operator, such that
\[
  F_i \& F_j = (F_i \cap F_j)\setminus (F_i \cap F_j)^\textsf{c} = \omega_{k},
\]
where $k$ is the index of the $k$\ts{th} intersection between $F_i$ and $F_j$.
\end{mydef}
...
all $2^N-1$ intersections into account.

\section{Initial layout}
\label{sec:initConfig}

For our initial layout, we employ a constrained version of
multi-dimensional scaling~(MDS) that has been adapted from \pkg{venn.js}~\citep{Frederickson_2016},
which in turn is a modification of an algorithm used in \pkg{venneuler}~\citep{Wilkinson_2012}.
In it, we consider the pairwise relationships between the sets, attempting to
position their respective shapes so as to minimize the difference between the
separation between their centers required to obtain an optimal overlap~($\omega$)
...
Setting $r_i = \sqrt{\omega_i/\pi}$, where $\omega_i$ is
the size of the $i$\ts{th} set, we are able to obtain $d$ numerically using
the squared difference between $O$ and $\omega$~(the desired overlap),
...
but often, compromises must me made. As with many of the
problems in this thesis, this turns out to be another optimization problem.
It can be
...
of squares of the optimal separation of sets, $d$, that we found
...
because the current release of R suffers from a bug that causes the
...
the Hessian offered by the optimizer\footnote{The current development version
of R features a fix for this bug; it is our intent to switch to using the
analytical Hessian as soon as it is introduced in a stable version of R.}.
\begin{fullwidth}
\begin{equation}
  \bm{H} = \smashoperator[l]{\sum_{1\leq i<j\leq N}}
  \begin{bsmallmatrix}
...
all the overlaps in the diagram. We initially restricted
...
As we briefly discussed in \autoref{sec:background}, we now need the ellipses'
...
<<intersect-1, fig.width=2, fig.height=2>>=
...
\par\medskip
\begin{subfigure}[b]{\linewidth}
<<intersect-2, fig.width=2, fig.height=2>>=
...
\medskip
\begin{subfigure}[b]{\linewidth}
<<intersect-3, fig.width=2, fig.height=2>>=
xyplot(
  y ~ x, data = ellipses, type = "l", groups = fac, asp = 1,
  xlab = NULL, ylab = NULL,
  scales = list(draw = FALSE, axes = FALSE),
...
\autoref{sec:intersecting-ellipses}, we can now find the
...
\item integrating the ellipse in [$0,\theta_0$] and [$0,\theta_1$], producing
...
and relative complements from~\cref{def:omega}. For this, we must also
decompose the overlap areas hierarchically, so that each area maps uniquely
...
\item spreading points across the ellipses using Vogel's
  method~(see \autoref{sec:labeling} for a brief introduction),
...
We feed the initial
layout computed in~\autoref{sec:initConfig} to the optimizer---once again
...
\label{eq:stress}
\end{equation}
where \[\beta = \frac{\sum_{i=1}^n A_i\omega_i}{\sum_{i=1}^n \omega_i^2}.\] This
is the equivalent of
fitting a linear regression through the origin, where $\beta$ is the
slope of the regression line~(\cref{fig:origin-regress}).

<<origin-regress, fig.env="marginfigure", fig.cap="Optimizing via stress is akin to fitting a least-squares linear regression through the origin.">>=
...
\autoref{sec:background} and covered it again in \autoref{sec:final-config}.
...
\pkg{venneuler}~(\cref{fig:venneulerHard1}).
...
the authors replaced 0 with 0.00001. Since the diagram was fitted with
...
used by \citet{Frederickson_2015a}.}
\label{tab:netflix}
\centering
\libertineTabular
...
Next, we compute the required areas, $\omega$~(\cref{def:omega}), for each
...
diagram as a success if its \emph{diagError} is lower than 0.01, that is,
...
successful diagrams, that is no wider than 0. We use the normal approximation
interval
\begin{equation*}
\mathcal{I}(\hat{p})_{0.95} = \hat{p} \pm z_{0.95}\sqrt{\frac{\hat{p}(\hat{p}-1)}{n}}.
\end{equation*}
The algorithm is formalized in~\cref{alg:consistency}. For circular diagrams,
...
\For{$N \gets 3:8$}
  ...
  \Do
    ...
    \ForAll{$j \gets$ software packages}
      ...
      \State $\Gamma(\hat{p}_j)_{0.95} \gets 2 \times z_{0.95}\sqrt{\frac{\hat{p}_j(\hat{p}_j - 1)}{i}}$
      \If{$\Gamma(\hat{p}_j)_{0.95} > 0.02$ \Or $i < 1000$}
        \State $A_{i_j} \gets \Call{FitDiagram}{\omega_i}$
    ...
    \EndFor
    \State $i \gets i + 1$
  \DoWhile{\All $\Gamma(\hat{p})_{0.95} > 0.02$ \Or $i < 1000$}
\EndFor
\end{algorithmic}
\end{algorithm}

\begin{figure*}[bhtp]
<<consistency, fig.height = 3, fig.width = 6.8, cache.extra = mtime(file.path("data", "data_consistency.rda"))>>=
...
success is defined as a diagram with a \emph{diagError}~\eqref{eq:diagError} below
0.01.}
...
\Sexpr{round(100*min(consistency_eulerr_ellipse$m_success), 0)}\%. \pkg{Vennerable}, which
...
In \autoref{sec:consistency}, we assessed the efficacy in reproducing diagrams
...
exact solution, one might still exist that we deem \emph{good enough},
...
$N=3,4,\dots,8$ sets and for a minimum of 1000 iterations, we initialize $2^N-1$
permutations of set combinations,
select one for each set, and initialize these to a number in
$\mathcal{U}(\epsilon, 1)$, where
...
one\footnote{The relevant line of code used to generate this $\epsilon$ is
\code{sqrt(.Machine\$double.eps)}}. After this, we pick $0 \text{ to } \binom{N-2}{1}$ elements
from the $2^N-N-1$ remaining permutations and assign to them a number from
$\mathcal{U}(0, 1)$ as before.

As in \autoref{sec:consistency}, we run our simulations until we achieve
...
he algorithm is formalized in~\cref{alg:accuracy}.

\begin{algorithm}[hbtp]
\caption{The algorithm we use to simulate random set relationships and fit
them with the software under study to assess their accuracy. $\Gamma(\bar{x})_{0.95}$
denotes a 95\% confidence interval around the mean \emph{diagError}, $s$ is the
corrected sample standard deviation, $F$ is the collection of sets in the
input, and $\epsilon$ is defined as the square root of the
...
\For{$N \gets 3:8$}
  ...
  \State $s \gets 0$
  \Do
    \State $\omega_i = \{\omega_{i_1},\omega_{i_2},\dots,\omega_{i_{2^N-1}} \} \gets 0$
    \For{$3:N$}
      \State $k \gets$ random index in $\{\omega_i : \omega_i \cap F_N \neq \emptyset\}$
      \State $\omega_{i_k} \gets \mathcal{U}(\epsilon, 1)$
    \EndFor
    \State $V \gets $ random indices from $\{\omega_i : \omega_i = 0\}$
    \State $\{\omega_i : \omega_i = v\} \gets \mathcal{U}(\epsilon, 1)$
    \ForAll{$j \gets$ software packages}
      \State $A_{j_i} \gets \Call{FitDiagram}{\omega_i}$
      \State $x_{j_i} \gets \Call{DiagError}{A_{j_i},\omega_i}$
      \State $s_j \gets \sqrt{\frac{1}{i-1}\sum (x-\bar{x})^2}$
      \State $\Gamma(\bar{x}_j)_{0.95} \gets 2 \times t_{0.95}s_j/\sqrt{i}$
    \EndFor
    \State $i \gets i + 1$
  \DoWhile{\All $\Gamma(\bar{x})_{0.95} > 0.02$ \Or $i < 1000$}
\EndFor
\end{algorithmic}
\end{algorithm}
...
\begin{figure*}[hbtp]
<<accuracy, fig.height = 3, fig.width = 6.6, cache.extra = mtime(file.path("data", "data_accuracy.rda"))>>=
...
diagerror <- filter(accint_summary, Metric == "diagError") %>%
  pull(median)
stress <- filter(accint_summary, Metric == "stress") %>% pull(median)
...
<<accuracy-int, fig.height = 2.1, fig.width = 4.4, cache.extra = mtime(file.path("data", "data_accuracy_int.rda"))>>=
...
\Sexpr{diagerror[1]} followed by \pkg{venneuler} at \Sexpr{diagerror[2]},
\pkg{eulerr} at \Sexpr{diagerror[3]}, \pkg{eulerAPE} at \Sexpr{stress[4]},
and \pkg{Vennerable} at \Sexpr{stress[5]}. As for stress, however, the order is
partially reversed with respective stress values at
\Sexpr{stress[1]}, \Sexpr{stress[2]}, \Sexpr{stress[3]}, \Sexpr{stress[4]},
\Sexpr{stress[5]} for
...
Using the same method as in \autoref{sec:accuracy}, we generate random set
...
last-ditch optimizer (\autoref{sec:last-ditch}) that is activated by default
if the fit is not error-free.

\begin{figure*}[hbtp]
<<performance, fig.height = 2.5, fig.width = 6.6, cache.extra = mtime(file.path("data", "data_performance.rda"))>>=
dd_perf <-
  data_performance %>%
  filter(!is.na(Time)) %>%
  mutate(Sets = as.factor(Sets), Time = Time/1e9)
...
(See \autoref{sec:last-ditch}). The web application
...
generated in \autoref{sec:consistency} within our margin of error.
...
initial optimizer. As we discussed in \autoref{sec:background},
...
time-consuming (as we saw in \autoref{sec:performance}), but if activated
...
as we briefly covered in \autoref{sec:background},
...
diagError\footnote{It might be appropriate to note that \pkg{eulerr} and \pkg{venn.js}
both attempt to minimize the sums of squares and not diagError.}; \pkg{venn.js}
scores lowest. Interestingly---as well as surprisingly, given the results
from \autoref{sec:consistency}---\pkg{venn.js} performs worse than \pkg{venneuler}
...
areas. For eight sets, for instance, this means investigating $\Sexpr{2^8-1}$
...
reproduce random elliptical diagrams (see \autoref{sec:consistency}), implying that the
performance in producing random set relationships (see \autoref{sec:accuracy})
...
\pkg{eulerr} need concern themselves with. In \autoref{sec:input}, we described
...
and \emph{diagError} metrics that were introduced in~\autoref{sec:gof}.

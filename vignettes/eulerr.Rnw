...
\begin{figure}[bhtp]
\sidecaption{Reproducibility tests for ellipses and circles generated from the
...
<<consistency, fig.height = 2.3, fig.width = 4.25, cache.extra = tools::md5sum(file.path("data", "data_consistency.rda"))>>=
data_consistency %>%
  spread(metric, loss) %>%
...
\pkg{venneuler}~\citet{wilkinson_2012}. We could try to fit the diagram using
ellipses instead.

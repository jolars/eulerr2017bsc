...
It is not obvious which of the two fits are better.
The stress and diagError from the \pkg{eulerr}
diagram~(\cref{fig:netflix-eulerr}) is
\Sexpr{netflix_eulerr$stress} and \Sexpr{netflix_eulerr$diagErro} respectively,
...
\Sexpr{netflix_eulerr$stress} and diagError of \Sexpr{netflix_eulerr$diagError}.
...
\Sexpr{vennjs_gof$stress} and diagError of \Sexpr{vennjs_gof$diagError}.
\label{fig:netflix-vennjs}}
\end{subfigure}
\caption{The Euler diagrams generated from the Netflix Prize data. Notice
the different choices made by the algorithms.}
\label{fig:netflix}
\end{figure*}

Another option is of course to try to fit the diagram with ellipses
instead~(\cref{fig:netflix-ellipses}). We are now able to reduce our diagError
to \Sexpr{netflix_eulerr_ellipses$diagError} and our stress to
\Sexpr{netflix_eulerr_ellipses$stress}, which is mostly the result of being
able to dispose of all the three-way intersections that were implicitly
set to 0 when we fit our diagram.

\begin{figure}[hbtp]
\sidecaption{The data from \cref{tab:netflix} visualized with ellipses.
\label{fig:netflix-ellipses}}
<<fig.width = 3.5, fig.height = 3.5>>=
plot(netflix_eulerr_ellipses, border = "grey40")
@
\end{figure}
...
<<consistency, fig.height = 3, fig.width = 6.8, cache.extra = tools::md5sum(file.path("data", "data_consistency.rda"))>>=
...
<<consistencyMetrics, include = FALSE, cache.extra = tools::md5sum(file.path("data", "data_consistency.rda"))>>=
...
vennerable_na <- data_consistency %>%
  filter(software == "Vennerable", is.na(diagError)) %>%
  summarize(n()) %>%
  as.vector()
@
...
\Sexpr{100*min(consistency_eulerr_ellipse$m_success)}\%. \pkg{Vennerable}, which
is only able to produce three-set diagrams, only produces accurate diagrams for
\Sexpr{100*consistency_vennerable_circle}\% of the random layouts and moreover
...
\begin{figure*}[hbtp]
<<accuracy, fig.height = 3, fig.width = 6.6, cache.extra = tools::md5sum(file.path("data", "data_accuracy.rda"))>>=
...
\begin{figure*}[hbtp]
<<performance, fig.height = 2.5, fig.width = 6.6, cache.extra = tools::md5sum(file.path("data", "data_performance.rda"))>>=
dd_perf <-
  data_performance %>%
  filter(!is.na(Time)) %>%
  mutate(Sets = as.factor(Sets), Time = Time/10e9)
...
performs on par with \pkg{eulerAPE} and better than all other packages
that were tested in this thesis. In addition, the method is
speedier than the competition for set relationships with up to six sets,
wherafter it performs worse than \pkg{venneuler}.

\pkg{eulerr} has perfect consistency for all the different circular diagrams
tested in the \nameref{sec:accuracy} section. No other package managed
to fit all of the combinations perfectly, although both \pkg{eulerAPE} and
\pkg{venn.js} has next-to perfect consistency and did not differ significantly
from \pkg{eulerr}. \pkg{venneuler} was not able to reproduce between 35 and 60\%
of the diagrams. Most of these diagrams feature disjoint or subset
relationships, which \pkg{venneuler} have problems fitting because of its
initial optimizer. As we discussed in the \nameref{sec:introduction},
eulerr, venn.js, and \pkg{venneuler} all use multi-dimensional scaling for
their initial diagram. venneuler, however, unnecessarily restricts the
locations of disjoint and subset circles. Given, for instance, two disjoint
sets, the algorithm will

attempt to place them tangent to one another or the
loss function in the optimizer will report loss.


disjoint and subset circles tangent and, respectively, at the exact
midpoint of . Yet, because we are indifferent about where in the space
...
\begin{figure}[htbp]
\sidecaption{The eight first colors of the default color palette.\label{fig:colorexample}}

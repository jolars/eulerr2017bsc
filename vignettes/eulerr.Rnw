...
with a polygon out of sides formed from joining the points with straight lines
...
we employ \code{nlm()} from \pkg{stats}. We now also provide the option to
use ellipses rather than circles, in which case we allow the shapes
...
The user is free to choose if and when to activate the
optimizer using standard arguments to the main function of the package.}
...
<<consistency, fig.height = 2.3, fig.width = 6.6, cache.extra = tools::md5sum(file.path("data", "data_consistency.rda"))>>=
...
ggplot(dd, aes(x = sets, y = success_rate, group = software, label = software)) +
  xlab("Sets") +
  ylab("Success rate (%)") +
  ...
  geom_line(cex = 0.5) +
  geom_point(cex = 0.75) +
  geom_dl(method = list("last.bumpup", dl.trans(x = x + 0.2), cex = 0.75)) +
  facet_grid(~shape) +
  xlim(c(3, 10)) +
  guides(color = FALSE, fill = FALSE)
  theme(plot.margin = unit(c(5.5, 7, 5.5, 5.5), "points"))
@
...
data_accuracy %>%
  ...
  geom_violin(scale = "width", width = 0.5, size = 0.25) +
  # geom_tufteboxplot(median.type = "line", whisker.type = "line", width = 6,
  #                   hoffset = 0, voffset = 0.005, stat = "boxplot",
  #                   outlier.shape = NA,
...
data_performance %>%
  ...
  # geom_tufteboxplot(median.type = "line", whisker.type = "line", width = 6,
  #                   stat = "boxplot", outlier.colour = "transparent",
...
<<twofits1, fig.width = 4.2, fig.height = 2.1>>=
p1
@
\subcaption{The default settings.}
\end{minipage}%
\begin{minipage}[b]{.6\linewidth}
<<twofits2, fig.width = 4.2, fig.height = 2.1>>=
p2
@

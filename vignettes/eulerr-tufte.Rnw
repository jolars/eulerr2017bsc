...
\begin{marginfigure}
<<impossible, out.width="\\linewidth", fig.height=4>>=
fig_impossible()
@
...
\begin{figure}[hbtp!]
<<polyarea, fig.height = 3, fig.width = 4>>=
fig_polyarea()
@
\label{fig:polyarea}
...
<<venn, fig.height = 2.7, fig.width = 6>>=
fig_venneulerHard()
@
...
%
<<consistency, fig.cap = "Reproducibility test of 100 iterations of 3 to 8 circles each.", fig.height = 3, fig.width = 6>>=
fig_consistency()
@
%

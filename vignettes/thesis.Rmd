...
**eulerr**, **venneuler**, **Vennerable**, and **d3VennR**. The latter is an
interface to the **venn.js** script that has been discussed previously, but 
because it features an outdated version and since it only produces
images as html, we refrain from using it in our results. Only **eulerr** and 
**venneuler** support more than 3 sets, which is why there are only 3--set
results for **Vennerable**.

These results were computed on a PC with the following specs and software
configuration:

* Microsoft Windows Pro 10 x64
* Intel Core^TM^ i7-4500U CPU @ 1.80GHz, 2 cores
* 8 Gb memory
* R 3.4.1, x64
...
```{r venneulerHard, fig.cap = 'A comparison of a Euler diagram generated by \\textbf{venneuler} with two generated from \\textbf{eulerr} with circles and ellipses respectively.', fig.width = 1.8, fig.height = 1.8, fig.show = 'hold', fig.subcap = c("A solution from \\textbf{venneuler} with a stress of 0.006", "A solution from \\textbf{eulerr} with circles, resulting in a stress of 0.004.", "Another solution from \\textbf{eulerr}, this time using ellipses, with a stress of 0."), crop = NULL}
#fig_venneulerHard()
set.seed(1)
...
Insert more comparisons.

## Consistency

To assess the consistency of **eulerr**, **venneuler**, and **vennerable**, we
generate $i = 3,4,\dots,8$ random circles with radius $r$ and $x$ and $y$
coordinates
\begin{align*}
r_i     & \sim \mathcal{U}(0.3, 0.6)\\
x_i,y_i & \sim \mathcal{U}(0, 1)
\end{align*}
through 100 iterators for each $i$. Next, we compute the required areas,
$\omega$ (Definition \@ref(def:omega)), for each iteration and 
then fit a Euler diagram using all of the packages mentioned above. Finally,
we compute and return the goodness of fit tests *stress* and *diagError*
that---since our Euler diagrams were formed from sampled circles---in fact
have perfect solutions (\@ref(fig:consistencyCircles)).

```{r consistencyCircles, fig.cap = "Reproducibility test of 100 iterations of 3 to 8 circles each.", fig.width = 6}
p <- lattice::xyplot(loss ~ it | sets + metric,
                     groups = software,
                     xlab = NULL,
                     ylab = NULL,
...
We repeat this experiment with ellipses instead of circles (\@ref(fig:consistencyEllipses)), this time with
semiaxes ($a$ and $b$), coordinates ($x$ and $y$), and rotation ($\phi$) drawn
from
\begin{align*}
x_i,y_i & \sim \mathcal{U}(0, 1)\\
...
```{r consistencyEllipses, fig.cap = "Reproducibility test of 100 iterations of 3 to 8 ellipses refitted with \\textbf{eulerr}. Note that the scale on the y-axis is different in this figure compared with the previous figure.", fig.width = 6, fig.height = 2.7}
p <- lattice::xyplot(loss ~ it | sets + metric,
                     xlab = NULL,
                     ylab = NULL,
                     type = "h",
                     par.settings = tactile::tactile.theme(fontsize = c(8, 4)),
                     subset = shape == "ellipse",
                     data = data_consistency)
latticeExtra::useOuterStrips(p)
```
...
which converges in $\mathcal{O}(2^n)$ time. Meanwhile @wilkinson_2012 report
convergence in $\mathcal{O}(n)$ time. The redeeming quality of **eulerr** lies
in its implementation in the C++ programming language via the **R** packages
**Rcpp** [@eddelbuettel_2011] and **RcppArmadillo** [@eddelbuettel_2014].

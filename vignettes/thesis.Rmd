...
[^1]: Data originally collected by the British Board of Trade in their investigation of the sinking of Titanic.
...
algorithms (Newton and Quasi-Newton). `nlm()` uses the Hessian in addition
to the gradient, which we presently compute numerically due to a bug
in the update of the analytic gradient in the software implementation, which
at the time of writing has only been fixed in the development version of **R**.
...
Other methods, however, use exact algorithms
[@micallef_2013; @frederickson_2016]. Based on the fact that
the overlap between ellipses can be represented as a convex polygon
with elliptical segments on the fringes (Figure \@ref(fig:polyarea)), we are
able to compute the area of the overlap to numerical precision. 
Since the polygon section is always convex, it is trivial to find its area using
the the triangle method.
...
This method, however, necessiates that we know the points
at which the ellipses intersect. **eulerr**'s approach to this is based
on a method outlined by @richter-gebert_2011 and moreover owes owes significant
debt to the **R** package **RConics** [@huber_2014], in which has been tremendously helpful
...
the intersection points, we first
...
* split this conic into a pencil of two lines, and finally
